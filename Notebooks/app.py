...
def user_input_features():
    ...
    COLOUR = st.sidebar.selectbox('Colour', np.sort(cars_final.Colour.unique()), index=-1)
    TYPE = st.sidebar.selectbox('Type', cars_final.Type.unique(), index=0)
    
    
    
    PROVINCE = st.sidebar.selectbox('Province', np.sort(cars_final.Province.unique()), index=1)
